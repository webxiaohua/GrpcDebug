...
layui.define(['jquery', 'laypage'], function (exports) {
    ...
    if ($('#ew-css-selectInput').length <= 0) {
        layui.link(layui.cache.base + 'selectInput/selectInput.css?v='+layui.cache.version);
    }
    // 加载全局鼠标位置
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (!__res.hasOwnProperty('data')) {
                        layui.hint().error('返回数据格式有误，必须满足如下如下格式：{"code":0,"msg":"success","data":[]}');
                        return false;
                    }
                    var data = __res.data;
    ...
    selectInput.prototype.bindEvents = function () {
        ...
        components.$inputElem.off('blur').on('blur', function () {
            ...
            layui.event.call(this, _MOD, 'itemBlur(' + components.filter + ')', commonMember(this));
        });
