    ...
    view.req = function(options){
      ...
      return $.ajax($.extend({
        ...
        ,error: function(e, code){
          ...
          //当页面返回400时，默认为api正常错误提示
          if(e.status == 400){
            if(e.responseJSON[response.statusName] == response.statusCode.logout){
              view.exit();
            }
            typeof options.done === 'function' && options.done(e.responseJSON);
            // console.log(response.statusName);
            // if(!view.inArray(e.responseJSON[response.statusName], response.indCode)){
